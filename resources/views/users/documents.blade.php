...
@section('content')
    ...
    @elseif (isset($all) != 0)
        <div id="All">
            <div class="container bg-light p-2 rounded mb-3">
                <div class="row">
                    <div class="col-md-3">
                        <form action="" class="search-form">
                            ...
                            <div class="input-group">
                                ...
                                <input type="text" class="form-control" placeholder="Reference Number" name="search"
                    ...
                    <div class="col-md-9 d-lg-flex d-md-flex d-sm-block d-xs-block justify-content-end">
                        ...
                        <li class="dropdown d-flex text-center">
                            <button class="btn btn-primary p-1 dropdown-toggle" type="button" id="dropdownMenuButton"
                                data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">
                                Sort by Date
                            </button>
                            <div class="dropdown-menu text-center">
                                <p class="p-2"><a href="{{ route('documents-list', ['sort' => 'asc']) }}">Ascending<i
                                            class="fa fa-arrow-up"></i></a></p>
                                <p class="p-2 mb-0"><a href="{{ route('documents-list', ['sort' => 'desc']) }}">Descending<i
            ...
            <div class="container">
                <div class="card col-md-12 bg-white">
                    <div class="mx-3 my-1 p-2">
                        <a href="add-document"><button class="btn btn-success float-end" data-toggle="modal"><i
                                    class="fas fa-plus"></i></i></button></a>
                        ...
                        <div class="card-body">
                            <div class="table-responsive">
                                <table class="table text-center">
                                    ...
                                    <tbody id="table-body">
                                        @foreach ($all as $doc)
                                            <tr data-office-name="{{ $doc->receiverOffice_id }}"
                                                data-document-type="{{ $doc->docType }}"
                                                ...
                                                @if ($doc->status == 12)
                                                    {{-- //rejected sent back --}}
                                                    <td><span class="badge bg-danger d-inline">Found Issue (by
                                                            Recipient)</span></td>
                                                @elseif ($doc->status == 11)
                                                    {{-- //rejected --}}
                                                    <td><span
                                                            class="badge bg-secondary d-inline">REJECTED(IRREDEEMABLE)</span>
                                                    </td>
                                                ...
                                                @elseif ($doc->status == 9)
                                                    {{-- //approved and kept --}}
                                                    <td><span class="badge bg-success d-inline">APPROVED and Kept</span>
                                                    </td>
                                                @elseif ($doc->status == 8)
                                                    {{-- //resubmitted --}}
                                                    <td><span class="badge d-inline"
                                                            style="background-color: rgb(48, 48, 185);">Resubmitted</span>
                                                    </td>
                                                @elseif ($doc->status == 7)
                                                    {{-- //resolved --}}
                                                ...
                                                @endif
                                                <td class="text-center"><a href="{{ url('qrinfo/' . $doc->referenceNo) }}"
                                                        title="Click for more Information"><i class="fa fa-link"
                                                            aria-hidden="true"></i></a></td>
                                                <td class="text-center">
                                                    {{ \Carbon\Carbon::parse($doc->created_at)->format('Y-m-d') }}</td>
                                                <td>
                                                    <form action="{{ route('download-qr-code') }}" method="get">
                                                        <input type="text" value="{{ $doc->referenceNo }}"
                                                            name="referenceNo" style="display:none;">
                                                        <button type="submit" class="btn btn-outline-dark"
                                                            title="Download the QR code">Download</button>
                                                    </form>
                                                </td>
                                            </tr>
                                        @endforeach
